...
<section class="page-head">
    <div>
        <h2>🚚 إنشاء أمر توريد</h2>
        <p>سجل شحنة من المخزن الرئيسي إلى فرع. يمكنك إنشاء أو تعديل أمر توريد هنا.</p>
    </div>
    <div>
        <a class="btn" href="index.php?r=transfers_supply">رجوع إلى سجل التوريدات</a>
    </div>
</section>

<form class="panel" id="transfer-form" method="post" action="index.php?r=transfers_supply">
    ...
    <div class="grid-form" style="margin-bottom: 16px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
        ...
        <label style="grid-column: span 2;">ملاحظات الشحنة
            <input name="notes" id="transfer-notes" placeholder="تفاصيل إضافية عن شحنة التوريد..." value="<?= $editMode ? e($transfer['notes']) : '' ?>">
        </label>
    </div>

    <h3 style="margin-top: 14px; margin-bottom: 8px; font-size: 13.5px; border-bottom: 1px solid var(--line); padding-bottom: 4px;">الأصناف والكميات المراد توريدها</h3>
    ...
    <div style="display: flex; gap: 10px; align-items: end; margin-bottom: 16px; max-width: 600px;">
        ...
        <button type="button" class="btn secondary" id="btn-add-transfer-item" style="height: 38px;">أضف للتوريد</button>
    </div>
    ...
    <button class="btn primary" id="btn-submit-transfer"><?= $editMode ? 'حفظ التعديلات' : 'إنشاء أمر التوريد' ?></button>
...
function resetTransferForm() {
    ...
    document.getElementById('btn-submit-transfer').textContent = 'إنشاء أمر التوريد';
